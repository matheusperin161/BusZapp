...
def send_password_reset_code_email(to_email: str, username: str, code: str) -> bool:
    """Envia e-mail com o código de 6 dígitos para redefinir a senha."""
    html = f"""
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
      <div style="background:linear-gradient(135deg,#fbbf24,#f97316);padding:32px 40px;text-align:center">
        <h1 style="color:#fff;margin:0;font-size:28px;font-weight:800">🚌 BusZapp</h1>
      ...
      <div style="padding:32px 40px">
        ...
        <div style="text-align:center;margin:0 0 28px">
          <div style="display:inline-block;background:#fffbeb;border:2px dashed #f59e0b;border-radius:14px;padding:18px 36px">
            <span style="font-family:'Courier New',monospace;font-size:38px;font-weight:800;letter-spacing:10px;color:#b45309">{code}</span>
          </div>
        </div>
        <p style="color:#718096;font-size:13px;line-height:1.5;margin:0">
          Este código expira em <strong>15 minutos</strong>. Se você não solicitou a redefinição, pode ignorar este e-mail.
        </p>
      </div>
      <div style="background:#f7fafc;padding:16px 40px;text-align:center;border-top:1px solid #e2e8f0">
        <p style="color:#a0aec0;font-size:12px;margin:0">BusZapp — Mobilidade Urbana</p>
    ...
    """
    return _send(to_email, f'{code} é o seu código de redefinição de senha — BusZapp', html)
